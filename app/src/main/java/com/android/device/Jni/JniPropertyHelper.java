...
public final class JniPropertyHelper {
    ...
    public static String getProcRedirectProbe() {
        ...
        try {
            String value = JniInterface.getProcRedirectProbe();
            if (value == null || value.trim().isEmpty()) {
                return "{\"error\":\"Error: native proc redirect probe returned empty\"}";
            }
            return value.trim();
        } catch (UnsatisfiedLinkError e) {
            jniUsable = false;
            Log.w(TAG, "Native proc redirect probe failed", e);
            return "{\"error\":\"" + JNI_UNAVAILABLE + "\"}";
        } catch (Throwable t) {
            Log.w(TAG, "Native proc redirect probe failed", t);
            return "{\"error\":\"" + toErrorMessage(t) + "\"}";
        }
    }
